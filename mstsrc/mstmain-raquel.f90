...
142     format(a11,1f6.3,1f4.1,1f6.1,1f6.3,1f8.2)
        ...
        tcore(1:mdelta) = (0.d0,0.d0)
      	stepdel = qmax/ndel
        write(61,*)'entering tntensor'
        call tntensor(w12,typev,kinem) !!!!! ATTENTION CHECK HERE
        STOP                           !!!!! ATTENTION CHECK HERE
c====================================================================
c check
...
!	real*8 hat(i) = sqrt(real(2*I+1))
	...
	rhocm(:,ll) = 0.0
        ...
        write(99,*)'Entering cmdens:mllmx=',mllmx
 ...
 400   continue
       ...
       function tctens(ib,ibb,i,j,ith)
         ...
         if (ib<=nint(2*s3)) then
            ...
            write(*,*)'tctens: error in tensor arguments!'
         end if
       end function tctens
...
1001   format(/," o Calling MSO with e14=",1f6.2," MeV")
       ...
       enddo
!! Raquel: por favor, verifica estas linhas, e apaga se nao
!! sao utilizadas (Antonio)
!! NOT USED??????
!----------------------------------------------------------
...
c scattering amplitscatteringude making the high energy assumption
...
c so we send as argument a modified c.m. energy, so the
...
! Use nangles from mst.in
            ...
            call readsmat(type,tnmat,kv,z2,muv,j2)
...
c ** KL
       ...
       write(61,*)'nangles',nangles
            write(61,*)'pause 3'  
            do 110iqrr = 1, nangles
               thet = thmin + dth*dble(iqrr-1)
               thrad = thet*pi/180.
               cthna = cos(thrad)
               qrr = sqrt(2*kv*kv*(1-cthna))
                write(61,*)iqrr,'qrr',qrr
 110            continue
           write(61,*)'pause 4' 

       do 111 nnn=1,nangles
          write(61,*)nnn,'xxq=', xxq(nnn)
...
c*****************************************************************************
       ...
       real*8 ::  th,racah2,cleb6,factnn,factnc,factna,hat2,hat
...
!***    write interpolated densities in q and th grid
 ...
 140    continue
	...
	q = radxisd(iq)       
     ...
     X    id=0,2 ),  ic=0,2 )
 145  continue
        write(23,*)'valence interpolated in theta grid'
        do 150  ith = 1,nangles
        th = thmin + dth*dble(ith-1)
